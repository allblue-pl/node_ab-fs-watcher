...
class Watcher
{
    ...
    _fsPaths_Get()
    {
        ...
        for (let pattern_fs_paths of pattern_fs_paths_array)
            fs_paths = fs_paths.concat(pattern_fs_paths);
    ...
    {
        let path_index = this._paths.indexOf(path);
        if (path_index === -1)
            return;

        this._paths.splice(path_index, 1);
    ...
    _fsPaths_Update(paths)
    {
        ...
        for (i = 0; i < this._paths.length; i++) {
            if (paths.indexOf(this._paths[i]) === -1) {
                this._fsPaths_Remove(this._paths[i]);
            }
        }

        for (i = 0; i < paths.length; i++) {
            if (this._paths.indexOf(paths[i]) === -1) {
